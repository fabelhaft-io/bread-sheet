...
**Leaving the rating flow (scan → rate → out):** the "Rating Submitted!" card is a *state* of `product/[barcode].tsx`, not a route, and the product screen sits on the root stack **above** the tabs. `router.back()` therefore drops the user wherever they entered from — after a scan, the camera still pointed at the barcode they just rated. The card's primary action is `router.navigate('/(tabs)')` (Home, where the new rating is listed) with a secondary "Back to product"; the card is also cleared by the focus-effect cleanup so a later visit never opens on a stale success screen. The other half of that loop lives in `app/(tabs)/scan.tsx`: a code already acted on is **suppressed while it keeps being seen** (`RESCAN_GAP_MS`, refreshed by every sighting *and* by re-focusing the tab or closing the manual sheet), so returning to the scanner cannot instantly re-open what the user was leaving. Checked before the 2 s burst lock; the `__DEV__` inject seam passes `deliberate` and is exempt.

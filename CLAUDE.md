...
npm run build              # Compile TypeScript to dist/ (tsc = TypeScript 7)
...
**TypeScript 7 side-by-side (`server/`, `agent-team/`):** the server builds and type-checks with TypeScript 7 (the Go port), but `typescript` in `server/package.json` is an alias for `@typescript/typescript6` and the real compiler is installed as `@typescript/native`. TypeScript 7.0 ships no programmatic API (that arrives in 7.1), so typescript-eslint hard-fails on it — the alias is the arrangement the TypeScript team documents for exactly this gap, and it keeps type-aware lint rules running against the 6.0 API. The bins do not collide: **`tsc` is TypeScript 7, `tsc6` is TypeScript 6**, so `build`/`typecheck`/`lint` need no changes. Collapse the alias back to a plain `"typescript": "^7.x"` once typescript-eslint supports TS >= 7.1. `agent-team/` has no ESLint setup and takes the plain 7.x dependency. `bread-sheet-app/` and `server/lambda/imageResizer/` stay on TypeScript 6 — the app because Expo SDK 57 pins its toolchain.

**Single `google-auth-library` (`server/package.json` → `overrides`):** `@google-cloud/vision` 6 pulls google-gax 6, which wants google-auth-library **^11**, while `@google/genai` still declares **^10.3.0**. Left alone, npm keeps both copies and the one `AwsClient` from `services/gcpWorkloadIdentity.ts` — shared by the Vision *and* the Gemini/Vertex client — no longer satisfies Vision's `ClientOptions.authClient` type. The override pins one copy at ^11 for both. Forcing genai past its declared range is safe because it uses exactly `new GoogleAuth(options)` and `googleAuth.getRequestHeaders()`, unchanged in 11.x. Do not remove the override without re-checking that Vision still compiles.

...
The same `AwsClient` instance is handed to the Gemini/Vertex client as well, which is why `server/package.json` pins a single `google-auth-library` (^11) in `overrides`: `@google-cloud/vision` 6 depends on it via google-gax 6, `@google/genai` still declares ^10.3.0, and two copies of the package mean two incompatible `AwsClient` classes.
